      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-ink/8 px-5 py-4">
          <div className="flex items-center gap-2">
            <span className="font-display text-lg text-ink">Mi Carrito</span>
            {count > 0 && (
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full px-6 text-center">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" className="text-ink/20">
                <circle cx="9" cy="21" r="1" />
                <circle cx="20" cy="21" r="1" />
                <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6" />
              </svg>
              <p className="mt-4 font-display text-lg text-ink">Tu carrito está vacío</p>
              ...
              <Link
                href="/catalogo"
                onClick={closeCart}
                className="mt-6 rounded-xl bg-ink px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-ink/80"
              >
                Explorar catálogo
              </Link>
            </div>
            ...
            <ul className="divide-y divide-ink/6 px-5">
              {items.map((item) => (
                <li key={item.varianteId} className="flex gap-4 py-4">
                  {/* Image */}
                  <div className="relative h-18 w-18 shrink-0 overflow-hidden rounded-xl bg-cream-dark">
                    {item.imagenUrl ? (
                      <Image
                        src={item.imagenUrl}
                        alt={item.nombre}
                      ...
                      <div className="grid h-full w-full place-items-center text-[10px] text-ink-soft/40">
                        KLP
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-1 flex-col">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button
                        ...
                        className="shrink-0 text-ink-soft/40 transition hover:text-ink"
                        aria-label="Eliminar"
                      >
        ...
        {items.length > 0 && (
          <div className="border-t border-ink/8 p-5 space-y-4">
            ...
            <button
              onClick={clear}
              className="w-full text-xs font-semibold text-ink-soft/60 underline underline-offset-4 transition hover:text-ink-soft"
            >
              Vaciar carrito
            </button>
          </div>
